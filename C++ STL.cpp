...
void explainVector()
{
    ...
    vector<int> v2(v1);    // {20, 20, 20, 20, 20}  (copies)
    ...
    vector<int>::iterator it = v.end(); //  Points to an item AFTER 40;

    vector<int>::iterator it = v.rend(); //  Points to an item BEFORE 10;  r=reverse

    vector<int>::iterator it = v.rbegin(); //  Points to 40
                                           //    use cbegin() or cend() for constant pointers which cannot be incremnted or decremented

    cout << v[0] << " " << v.at(0);

    cout << v.back() << " "; //  Last element=40
